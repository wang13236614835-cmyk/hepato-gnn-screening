...
> **一分钟先看这里**：本库现在是 **GNN 方法学习 + 历史模型复核 + 组员任务工作区**。AIDD 主库是唯一正式科研状态源；`candidate_release=false`，GNN `gated/closed`，FASN Gate T1 待完成。旧 Top-10、旧 AUC、旧 verify、旧双靶和旧 docking 排名只作历史复核，不代表当前科研结论。

| 你要回答的问题 | 统一答案 |
|---|---|
| 我们现在研究什么？ | AIDD 主库的 MASH 研究链重新核对；本库负责方法、历史和组员工作，不创建第二套候选链。 |
| 当前做到哪一步？ | ④ 化学空间外推与泄漏审计；FASN Gate T1（多 assay 对齐）是当前阻塞。 |
| 普通组员先做什么？ | 打开自己的“当前任务”卡；再按需要进入历史复核任务。 |
| 做完交什么？ | 带来源、日期、审核人和状态的记录；不能只报一个分数。 |
| 最终状态去哪看？ | AIDD 主库 `qualification_v1/RESEARCH_STATE.md`；本库 `RESEARCH_STATE.md` 是角色镜像。 |

**任务分两类：**

- **当前任务**：服务 2026-09-14 这一轮状态重核对（状态、结构/标签、对接失败证据、文献 claim→source）。
- **历史复核任务**：复现旧代码/旧输出/教学流程；旧数字可以保留，但不能升格为当前科研结论。
...
通用五步（每人相同，细节见自己那节）：
① 先读本页“当前研究工作台”和自己的“当前任务”卡 → ② 读 `RESEARCH_STATE.md` 与 AIDD 主库只读状态
→ ③ 先完成当前任务，再按需要打开“历史复核任务”→ ④ 交付记录必须带来源、日期、审核人和状态
→ ⑤ 每周在自己打卡页记录学习；学习打卡不等于科研资格签核。

历史核验仍可按 `docs/VERIFY_TASKS.md` / `docs/VERIFY_MANUAL.md` 执行，但旧 Top-10、旧 AUC、旧 45 项和旧 docking 只作历史快照，不代表当前科研结论。

---

## 王启龙（负责人 · 工程支撑）

> **当前任务**：汇总主库状态、资产保留/降级和成员人工审核；下面 L1–L6 是历史复核/工程记录，不是当前候选发布任务。
...
> **当前任务**：运行安全默认入口与软件回归，确认 GNN 代码/历史结果没有越界；下面 N1–N7 是历史复核/学习记录。
...
> **当前任务**：逐条核对结构/标签注册表，保持 pending_review，不自动升级；下面 Y1–Y7 是历史复核/学习记录。
...
> **当前任务**：复核历史 docking 输入、协议与失败门控，不生成新排名；下面 D1–D7 是历史复核/学习记录。
...
> **当前任务**：核对主库 claim→source/PMID 证据链；旧 Top-10 只作历史复核；下面 M1–M5 是历史复核/学习记录。
