...
仓库内全部36个 md 文档的PDF镜像（目录结构与源一一对应；本次新增
VERIFY_TASKS、MEMBER_WORKBENCH、负责人推进程序、exe使用说明、五人工作台），由
...
| 文件夹/文件 | 负责人 | 内容 | 核验方法 |
|---|---|---|---|
| 王启龙/ 宁显泷/ 衣思淼/ 代维斯丹/ 王散曼/ | 各自成员 | 每人两件：工作台.md（我是谁/任务勾选表/名下文件/公用资源指引/操作五步/完成记录）＋打卡.html（学习·打卡·周报，浏览器打开） | 任务行数=VERIFY_TASKS 对应小节（6/6/5/4/5）；文件数组=附录A 该线（12/90/14/18/4）；打卡页首屏姓名与文件夹名一致；与 docs/ 公用总表不矛盾（改公用表须同步个人份） |
