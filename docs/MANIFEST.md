...
| 学期推进流五页 v5（权威来源纪律版） | 王启龙 | 老师要求"从公开权威数据库查找"落进全队规划：①资源组①升级为「公开权威数据库白名单」②学习资源页新增「🏛 权威来源纪律」卡：六类信息（结构/活性/蛋白/中药/成药性/文献）×只认库名×入库三要素（库名+唯一编号+访问日期）表＋四条硬规则（二手网页只当线索；三要素缺一不可；AI 只出检索词不下结论、须回权威库核实；非权威源数据重查或删并留痕）③页脚加第二条纪律；配套 WP3 新增"来源纪律"节＋验收第 1 条加三要素、VERIFY_TASKS 第 0 节加全队纪律声明 | 学习资源页可见"权威来源纪律"卡与白名单组名；页脚含"公开权威数据库"；WP3 验收第 1 条含三要素；五页一致（改模板后重跑生成器） |
