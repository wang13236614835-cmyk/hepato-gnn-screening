...
| 学期推进流五页 v9.2（MOOC卡链接+去重复+挂第1周版） | 王启龙 | ①M01–M08 卡改带课程直达链接(icourse163·FUDAN-1485348164)，渲染标签"官方文档↗"改"直达↗"；②删除与单元卡重复的 R-W0-01 总入口卡；③M卡挂第1周(wk:1)——开学前"预习第1周"与W1周卡片"本周视频"直接可见，开学前(至09/06)即可勾；④卡面文案转义引号改「看过」避免源码视图出现 \"  | W1周卡片"本周视频"含8张MOOC卡且可点链接；学习资源无重复MOOC卡；RES数组合法 |
| 学期推进流五页 v9.3（去learn重复+M09+文件指南版） | 王启龙 | ①周卡片 learn 条目与同周视频卡重复的删除（龙6/宁4/代1条），概念型条目保留；②MOOC 补 M09（第12章+附录·答辩文献清单），9卡覆盖课程全部12章+附录；③新增 docs/文件使用指南.md（总纲/技术口径/数据结果/工具学习四类：是什么·怎么查·推荐方法·注意），五页工作台链接入口 | W1周卡片无与视频卡重复的learn条目；资源页M09在；工作台含文件指南链接 |
| 学期推进流五页 v9.4（MOOC按周分布版） | 王启龙 | ①9张MOOC卡(R-M01…M09)按12章×16周映射分布到 W1/W3/W4/W5/W7/W10/W11/W13/W14——相关的优先、当周理论指导当周实践；开学前先看M01余力M03；②卡面瘦身三段式"章·主题→指导本周→教回题"，完成规则只在M01出现一次；③学习任务.md第〇节改按周分布表，12章表第10/11章行与卡对齐 | 任一成员页周卡片：W1只见M01、W5见M05；学习资源按周分组；卡面单行简洁 |
