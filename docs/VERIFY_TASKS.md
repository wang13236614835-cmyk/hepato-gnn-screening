...
> **版本边界**：本表主要核验暑假 GNN 教学/审计快照，不是当前 AIDD 论文的放行清单。表中的旧 Top-10、45 项、59/14/18 划分和旧指标只用于复现对应历史版本；不满足或超过旧数字都不自动判错，也不作为“必须达到”的论文门槛。当前论文验收以 `docs/打卡修缮/验收规则.md` 和 AIDD 投稿任务台账为准，软件通过、数据审核、科学验证、真人复核、导师放行分开记录。

> 背景：暑假阶段全部结果（文献整理稿、数据集、模型打分、Top-60 排名）由
> AI 辅助流水线产出。在进入当前论文证据链之前，
...
`results/rankings/final_ranking.csv` 的历史 Top-10（final_score 降序，仅用于核对该版本）：
...
合理性旁证（历史快照）：临床金标准水飞蓟宾落第 7（§3.5）；阳性参照药奥贝胆酸
（NV-011）被适用域标记"域外"且 pred_mean≈0.26，未被顶替进榜单。
**上述数字全部未经真人核验，核验通过前仅称“历史 AI 初筛结果”；即使核验通过，也只说明该版本可复现，不构成当前候选、药效或机制结论。**
...
| 成员 | 校验线 | 一句话职责 | 完成时限 |
|---|---|---|---|
| 宁显泷 | 代码验证（主核） | 证明"代码跑得对、结果可复现" | W1 周日 9/13 |
| 王启龙 | 辅助 | 环境/汇总/固化标签，全流程托底 | W2 周日 9/20 |
| 衣思淼 | 数据线 A：数据集与排名表 | 证明"数字没被编造" | W1 周日 9/13 |
| 代维斯丹 | 数据线 B：对接·指纹·图 | 证明"打分与图忠实于代码输出" | W1 周日 9/13 |
| 王散曼 | 文献 | 证明"每条证据可溯源" | W2 周日 9/20（§4 为 WP5 任务，可顺延至 W14 全量） |
...
| # | 任务 | 预期（来源） | 交付物 |
|---|---|---|---|
| N1 | §2.1 历史教学复现 `python run_all.py --legacy-demo` | 历史快照的12条关键行逐字核对，含60候选入库；差异登记，不作当前论文门槛（VERIFY_MANUAL §2.1）；默认 `python run_all.py` 不运行旧流程 | 控制台输出粘贴本文件末尾记录表 |
...
| # | 任务 | 预期（来源） | 交付物 |
|---|---|---|---|
| L1 | §0/§1 环境与仓库完整性预检；为任何成员排障 | 默认 `python run_all.py` 应只显示未放行状态；历史复现使用 `--legacy-demo`（§0–§1） | 排障记录 |
...
| L5 | A/B 级问题的工程修复与全量复跑 | 历史库 verify.py 45 项与最终包 verify_fixes.py 20 项分别记录；不把两者当当前论文门槛 | 修复说明 + 复跑输出 |

### 2.3 衣思淼 — 数据线 A（数据集与排名表）

| # | 任务 | 预期（来源） | 交付物 |
|---|---|---|---|
| Y1 | §3.4 原始表抽查 `data/raw/tcm_seed_compounds.csv` | 历史快照89行；HP 48 / DC 40；HP-015 label=1、evidence=A（§3.4）；新库按审核结果更新 | 验算留痕（Excel 筛选截图或手算页拍照入 `results/logs/`） |
| Y2 | 划分隔离抽查 `data/splits/` | 历史快照中黄芩苷所在 scaffold 不出现在 test/val，train/val/test=59/14/18（§3.4）；仅核对来源与差异，不作样本量门槛 | 同上 |
| Y3 | §3.5 排名总表核验 | 历史快照的61行、rank 1–60和旧Top-3/水飞蓟宾/NV-011字段逐项核对；不构成当前候选 | 同上 |
| Y4 | 融合公式手工验算 ≥3 行（含历史第1、7、10名） | 与历史 final_score 差≤±0.002并记录原因；黄芩苷≈0.9291只作版本锚点 | 计算过程留痕 |
...
| D2 | 演示对接分核对：重跑 mock_docking | 历史演示分 -5.46 vs -5.05（差距0.42）仅作可复现性锚点，不作物理药效证据（§2.1 [docking] 行） | 输出留痕 |
| D3 | §3.6 四张图与 CSV 对照 | 历史 top_candidates.png 顺序、可靠性图与旧 ECE=0.210 逐项对账；不作当前模型放行条件（§3.6） | 逐图结论一行 |
...
| M2 | 历史 Top-10 证据专查：第 0 节 10 个化合物逐条对照 literature/01 | 证据等级、来源草药描述两边一致；**重点 NV-009 松脂素、NV-010 落叶松脂素**；不把历史榜当当前候选 | 逐条结论一行 |
| M3 | literature/01 头部"引用为整理稿"声明处理 | M1/M2 完成后更新声明口径（§4：进入论文引用前必须完成） | 修改后的声明 |
...
| 路径 | 任务号 | 看什么 |
|---|---|---|
| run_all.py | N1/N2 | `--legacy-demo` 重跑历史 12 关键行一致；两次 md5"一致"；默认入口不写旧 results |
